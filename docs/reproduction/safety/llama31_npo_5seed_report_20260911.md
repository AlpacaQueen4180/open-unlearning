...
Retain90 的 harmfulness 平均為 34.56%，五個 seed 分別為 19.53%、51.03%、58.12%、17.85%、26.28%。雖然全部低於公開 full baseline 70.89%，但 40.27 個百分點的跨 seed範圍顯示 safety outcome 很不穩定。FQ 也只有 3/5 seeds 為 p>0.05，因此目前最合理的表述是：**retain90 穩定避免崩潰，但 forgetting 與 safety 改善幅度都不穩定。**

## 五、Category-level 結果

Aggregate 僅保存 category ID，未包含類別名稱，因此以下以 ID 報告。百分比仍以該 category 成功取得 Judge 標籤的回答為分母。

| 設定 | Harmfulness 最高的三類（五-seed平均） | 同類平均 degeneration |
|---|---|---|
| 自製 f05, no retain | ID 7: 67.29%；ID 6: 65.29%；ID 9: 55.33% | 42.14%；32.00%；39.33% |
| 自製 f05, +retain95 | ID 9: 95.33%；ID 7: 95.17%；ID 6: 92.48% | 3.33%；0.69%；6.21% |
| 公開 f01, no retain | ID 9: 96.00%；ID 4: 92.96%；ID 6: 81.77% | 均為 0% |
| 公開 f01, +retain99 | ID 9: 95.33%；ID 4: 90.52%；ID 6: 81.72% | 均為 0% |
| 公開 f10, no retain | 所有類別均為 0%* | 所有類別均為 100%* |
| 公開 f10, +retain90 | ID 9: 56.67%；ID 6: 46.67%；ID 7: 46.19% | 2.00%；0%；1.43% |

\* 全面 collapse，不能按 safety 改善解讀。

Category 4 的 Judge coverage 特別需要注意：公開 f01 without-retain 與 +retain99 在五個 seeds 各有 63/150 與 67/150 筆 policy-blocked。其 92.96% 與 90.52% harmfulness 點估計只反映成功列，未知列比例很高。Category-level 主表應同時呈現 coverage 或 bounds，不能只列點估計。
...
7. Category-level aggregate 沒有保存人類可讀類別名稱，因此目前只按 ID 報告。
